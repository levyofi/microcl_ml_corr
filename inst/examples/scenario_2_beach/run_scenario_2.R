...
if (pkg_examples_dir != "") {
  ...
  SCENARIO_DIR <- system.file("examples", "scenario_2_beach", package = "microclCorr")
  OUTPUT_DIR   <- file.path(getwd(), "scenario_2_beach_results")
} else {
...
tasks <- list(
  list(
    name = "Ashkelon_15_m", site = "Ashkelon 15 m", title = "Beach (Ashkelon 15m)",
    train_blocks = c(0, 1, 3, 4, 5, 6, 7, 8, 9, 10), val_blocks = c(11), test_blocks = c(2)
  )
)

# Loader
load_beach_data <- function(path, site) {
  data <- load_prepared_csv_data(
    path, is_continuous_microhabitat = FALSE,
    datetime_format = "%Y-%m-%d %H:%M:%S", includes_index = TRUE
  )
  if ("microhabitat_sun" %in% names(data)) {
    data$microhabitat_sun <- NULL
  }
  data <- data[data$time > as.POSIXct("2025-05-25 00:00:00", tz = "UTC") &
               data$time < as.POSIXct("2025-08-26 00:00:00", tz = "UTC"), , drop = FALSE]
  data <- data[data$time_series_site == site, , drop = FALSE]
  list(data = data, site_col = "time_series_site")
}

# 1. Run Experiments
cat("\n=== RUNNING SCENARIO 2 EXPERIMENTS ===\n")
results_list <- list()
...
for (task in tasks) {
  ...
  cat(sprintf("\nProcessing Beach Logger: %s\n", task$name))
  ld <- load_beach_data(BEACH_PATH, task$site)
  site_data <- ld$data
...
p_pred <- ggplot(plot_df, aes(x = time)) +
  ...
  labs(title = "Coastal Beach (Ashkelon 15m)", x = NULL, y = "Temperature (°C)", color = NULL) +
...
ggsave(file.path(SCENARIO_DIR, "prediction_examples_beach.png"), p_pred, width = 8, height = 4.5, dpi = 300)
...
p_lc <- ggplot(summary_perc, aes(x = perc, y = mean_rmse, color = model, fill = model)) +
  ...
  labs(
    title = "Learning Curves: Coastal Beach (Ashkelon)",
...
ggsave(file.path(SCENARIO_DIR, "learning_curves_beach.png"), p_lc, width = 8, height = 4.5, dpi = 300)
...
markdown_table <- "| Model | Baseline NicheMapR RMSE (°C) | Corrected RMSE (°C) | Error Reduction (%) |\n"
markdown_table <- paste0(markdown_table, "| --- | --- | --- | --- |\n")
for (i in seq_len(nrow(summary_table))) {
  row <- summary_table[i, ]
  markdown_table <- paste0(markdown_table, sprintf(
    "| %s | %.3f | %.3f | %.1f%% |\n",
    row$model, row$mean_rmse_base, row$mean_rmse_corr, row$mean_improvement_pct
  ))
}
...
writeLines(report_content, file.path(SCENARIO_DIR, "scenario_2_report.md"))
cat("\n=== Scenario 2 Run Finished Successfully ===\n")
