...
for (site in sites_b) {
  ...
  hist_panels[[site]] <- make_residual_hist(site_df, title_str, xlim = xlim_site,
                                             show_strip = (site == sites_b[length(sites_b)]),
                                             show_legend = TRUE)
}

ncols_b <- min(2, length(sites_b))
       p_hist <- ggarrange(plotlist = hist_panels, labels = paste0("(", letters[1:length(hist_panels)], ")"), label.x = 0.15, label.y = 0.9, font.label = list(size = 24), ncol = ncols_b, nrow = ceiling(length(hist_panels) / ncols_b), common.legend = TRUE, legend = "top")

ggsave(file.path(SCENARIO_DIR, "residual_histogram_beach_pooled.jpg"),
       p_hist, width = 10, height = ceiling(length(hist_panels) / ncols_b) * 5.5, bg = "white", dpi = 300)
cat("Scenario 4 histograms created successfully!\n")
