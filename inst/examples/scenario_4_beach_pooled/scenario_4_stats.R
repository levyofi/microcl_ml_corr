...
lstm_2h      <- lstm_specific_preprocessing(scaled$train, scaled$val, scaled$test, window_size = 2, ts_names_col = SITE_COL)
rf_test        <- align_test_sets(splits$test, lstm_2h$test_dict, lstm_2h$index_info, SITE_COL)
...
for (i in seq_along(lstm_2h$index_info$datasets)) {
  site <- lstm_2h$index_info$datasets[i]
  idx_lstm <- lstm_2h$index_info$test_indices[[i]] + 1
...
write.csv(do.call(rbind, eval_rows), file.path(RESULTS_DIR, "beach_pooled_results.csv"), row.names = FALSE)
write.csv(do.call(rbind, daily_rows), file.path(RESULTS_DIR, "daily_extreme_rmse.csv"), row.names = FALSE)
write.csv(do.call(rbind, pred_dfs), file.path(RESULTS_DIR, "scenario_4_predictions.csv"), row.names = FALSE)
cat("Scenario 4 stats complete.
")
