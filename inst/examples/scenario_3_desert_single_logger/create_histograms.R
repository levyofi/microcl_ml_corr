...
for (task in tasks_s3) {
  ...
  hist_panels[[task$name]] <- make_residual_hist(full_df, task$title, show_legend = TRUE)
}

p_hist <- ggarrange(plotlist = hist_panels, labels = paste0("(", letters[1:length(hist_panels)], ")"), label.x = 0.15, label.y = 0.9, font.label = list(size = 24), ncol = 2, nrow = 1, common.legend = TRUE, legend = "top")

ggsave(file.path(SCENARIO_DIR, "residual_histogram_desert.jpg"),
  p_hist,
  width = 10, height = 5.5, bg = "white", dpi = 300
)
cat("Scenario 3 histograms created successfully!\n")
