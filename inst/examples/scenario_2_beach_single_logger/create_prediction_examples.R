...
p_excerpt <- make_pred_plot(head(full_df, 120), NULL, show_legend = TRUE) +
  ggplot2::guides(color = ggplot2::guide_legend(nrow = 3, byrow = TRUE)) +
  ggplot2::theme(legend.text = ggplot2::element_text(size = 10), legend.key.size = ggplot2::unit(1, "lines"))

p_arranged <- ggpubr::ggarrange(p_excerpt, label.y = 0.9, font.label = list(size = 24), ncol = 1, nrow = 1, common.legend = TRUE, legend = "top")

ggsave(file.path(SCENARIO_DIR, "prediction_examples_beach.jpg"),
  p_arranged,
  width = 5, height = 5.5, bg = "white", dpi = 300
)
cat("Scenario 2 prediction examples created successfully!\n")
