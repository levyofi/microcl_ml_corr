...
for (site in sites_d) {
  ...
  if (site %in% sites_d_sample) {
    excerpt_panels[[site]] <- make_pred_plot(head(full_df, 120), title_str, show_legend = TRUE)
  }
}

p_excerpt <- ggarrange(plotlist = excerpt_panels, labels = paste0("(", letters[1:length(excerpt_panels)], ")"), label.x = 0.1, label.y = 0.9, font.label = list(size = 24), ncol = 2, nrow = ceiling(length(excerpt_panels) / 2), common.legend = TRUE, legend = "top")

ggsave(file.path(SCENARIO_DIR, "prediction_examples_desert_pooled.jpg"),
  p_excerpt,
  width = 15, height = ceiling(length(excerpt_panels) / 2) * 5.5, bg = "white", dpi = 300
)
cat("Scenario 6 prediction examples created successfully!\n")
